...
use ggez::{
    ...
    graphics::{self, Color, DrawMode, FontData, PxScale, Rect, TextFragment},
...
const GATE_WIDTH: f32 = 100.;
const STEERING_SPEED: f32 = 110. / 180. * PI;
const MAX_ANGLE: f32 = 75. / 100. * PI;
...
fn play_sound(ctx: &mut Context, sound: &mut audio::Source, volume: f32) {
...
impl Screen {
    fn new(ctx: &mut Context) -> GameResult<Screen> {
        ...
        ctx.gfx
            .add_font(&s.font, FontData::from_path(ctx, "/font.ttf")?);
        Ok(s)
    }
    ...
    fn steer(&mut self, ctx: &mut Context, side: f32) {
        if side == 0. {
            return;
        }
        ...
        if self.direction > MAX_ANGLE {
            ...
            play_sound(ctx, &mut self.whoosh_sound, self.forward_speed * 0.005);
        }
    }
}
...
impl EventHandler for Screen {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        while timer::check_update_time(ctx, DESIRED_FPS) {
            ...
            self.steer(ctx, self.input.to_turn);
            match self.mode {
                Mode::Ready => {
                    if self.input.started {
                        self.mode = Mode::Running;
                        self.race_start_time = now;
                        play_sound(ctx, &mut self.click_sound, 1.);
                    }
                }
                Mode::Running => {
                    ...
                    if ski_tip_along <= next_gate_along {
                        if !self.entered_gate {
                                ...
                                || ski_tip_across > right_pole_offset
                            {
                                ...
                                play_sound(ctx, &mut self.bump_sound, 1.);
                            } else if self.disappeared_gates == TOTAL_N_GATES - 1 {
                                self.mode = Mode::Finished;
                                play_sound(ctx, &mut self.two_notes_sound, 1.);
                            }
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        canvas.draw(
            ...
            graphics::DrawParam::new()
                .dest(Point2 {
                    x: SCREEN_WIDTH / 2. + self.ski_across_offset,
                    y: SCREEN_HEIGHT * 15. / 16. - SKI_LENGTH / 2. - SKI_TIP_LEN,
                })
